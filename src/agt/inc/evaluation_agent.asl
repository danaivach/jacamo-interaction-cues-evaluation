...
+!start_evaluation(EnvUrl, EnvName): evaluation(scalability, PlanNum, MinSigNum, MaxSigNum) <-
   .print("Evaluation-scalability, MinSigNum:", MinSigNum, ", MaxSigNum:", MaxSigNum);
    ...
    makeArtifact("logger", "eval.TimeLogger", [MinSigNum, "scalability", PlanNum], LoggerId);
    makeArtifact("conf", "eval.ScalabilityConf", [EnvUrl, EnvName, "test", PlanNum, MinSigNum, MaxSigNum, true], ConfId);
    linkArtifacts(ConfId, "conf-out", LoggerId);
    focus(ConfId).

@agent_metadata_update[atomic]
+agent_metadata([KnownPlan], AbilityType, 0) : true <-
    -+ability(AbilityType);
...
+agent_metadata([], AbilityType, 1) : true <-
    -+ability(AbilityType).

@agent_metadata_many_plans_update[atomic]
+agent_metadata(KnownPlans, AbilityType, 1) : true <-
     -+ability(AbilityType);
    .relevant_plans({+!test_goal}, _, LL);
    .remove_plan(LL);
    .add_plan(KnownPlans);
    setPlanAdditionState(true).
